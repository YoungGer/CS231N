...
def batchnorm_backward(dout, cache):
  ...
  dmu = np.sum(-dx_hat/np.sqrt(var+eps), axis=0) - 2*dvar*np.mean(x-mu, axis=0)

  dx = dx_hat/np.sqrt(var+eps)+2*dvar*(x-mu)/len(x)+1.0*dmu/len(x)
    ...
    for fi in range(F):
      for hi in range(out_H):
        for wi in range(out_W):
          xi = x_pad[ni,:,hi*stride:hi*stride+HH,wi*stride:wi*stride+WW]
          ki = w[fi,:,:,:]
          bi = b[fi]
          out[ni,fi,hi,wi] = np.sum(xi*ki) + bi
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b, conv_param)
    ...
    for fi in range(F):
      for hi in range(out_H):
        for wi in range(out_W):
          ...
          bi = b[fi]
          # calculation
          douti = dout[ni,fi,hi,wi] 
          dx_pad[ni,:,hi*stride:hi*stride+HH,wi*stride:wi*stride+WW] += douti*ki
          dw[fi,:,:,:] += douti*xi
          db[fi] += douti
...
def max_pool_forward_naive(x, pool_param):
  ...
  for ni in range(N):
    for fi in range(C):
      for hi in range(out_H):
        for wi in range(out_W):
          subset = x[ni,fi,hi*stride:hi*stride+pool_height,wi*stride:wi*stride+pool_width]
          out[ni,fi,hi,wi] = np.max(subset)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
  ...
  H = (out_H-1)*stride+pool_height
  W = (out_W-1)*stride+pool_width
  dx = np.zeros([N,C,H,W])
    ...
    for fi in range(C):
      for hi in range(out_H):
        for wi in range(out_W):
          ...
          dx[ni,fi,hi*stride:hi*stride+pool_height,wi*stride:wi*stride+pool_width][subset==np.max(subset)] = douti
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
