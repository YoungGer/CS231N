...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    W1 = np.zeros([F, C, HH, WW])
    for fi in range(F):
        for ci in range(C):
            W1[fi,ci,:,:] = weight_scale*np.random.randn(HH,WW)
    b1 = np.zeros(F)
    ...
    out_H = 1 + (out_H - pool_height) / stride
    out_W = 1 + (out_W - pool_width) / stride

    W2 = weight_scale * np.random.randn(F*out_H*out_W, hidden_dim)
